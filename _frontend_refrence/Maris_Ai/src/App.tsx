...
import { CenterVisualization } from './components/CenterVisualization';
...
import { GlobeControlToolbar } from './components/GlobeControlToolbar';
...
export default function App() {
  ...
  const [targetRegion, setTargetRegion] = useState<'india' | 'atlantic' | null>(null);
          ...
          {(activeNav === 'home' || activeNav === 'incidents') && (
            <div className="flex-1 flex flex-col min-h-0 w-full h-full relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col h-full w-full pointer-events-none overflow-hidden justify-between p-2 sm:p-3 lg:p-4 gap-2.5">
                {/* Top KPI Ribbon (Explicit User Requirement) */}
                ...
                {/* Middle Row: Left Incidents Panel + Empty Globe View Area + Right Intelligence Panel */}
                <div className="flex-1 flex items-stretch justify-between min-h-0 gap-3 pointer-events-none">
                  ...
                  <div className="flex-1 pointer-events-none flex flex-col items-center justify-between min-w-0 h-full">
                    {/* Globe Layer & Navigation Toolbar: Sits directly below the KPI Ribbon with clean spacing */}
                    ...
                    {/* Center space remains completely transparent for direct 3D globe drag & zoom */}
